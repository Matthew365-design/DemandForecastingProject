...
K_DAYS_AVG = 0          # 0 = pure seasonal-naive; >0 = average last K days per slot
# -------------------------------------------------------------------
...
def load_and_regularise(path: Path) -> pd.Series:
    """
    Robust loader:
      - Print sheet/columns glimpse
      - Handle comma decimals
      - Parse dates flexibly
      - Regularise to 30-min grid and interpolate
    """
    # Try reading with comma decimals (common in LATAM/ES excel)
    df = pd.read_excel(
        path,
        sheet_name=SHEET_NAME,
        engine="openpyxl",
        usecols=[TIME_COL, VALUE_COL],
        # If commas are used for decimals, this keeps numbers correct
        decimal=",",
        dtype={VALUE_COL: "float64"},
    )

    # Debug prints to verify headers and sample rows
    print("[debug] columns:", list(df.columns))
    print("[debug] head:\n", df.head(3))

    # Parse timestamp robustly
    t = pd.to_datetime(df[TIME_COL], errors="coerce", dayfirst=True, infer_datetime_format=True)
    v = pd.to_numeric(df[VALUE_COL], errors="coerce")

    s = pd.Series(v.values, index=t, name=VALUE_COL).dropna()
    s = s.sort_index()

    # If duplicates, aggregate by mean
    ...
    if len(dt):
        print(f"[grid] min_step={dt.min()}, max_step={dt.max()} (should be 30min)")
    else:
...
def seasonal_naive_forecast(series: pd.Series, steps: int, period: int, k_days_avg: int = 0) -> pd.Series:
    ...
    If not, fall back to last-value persistence (simplest possible).
    """
    last_stamp = series.index[-1]
    future_index = pd.date_range(start=last_stamp + pd.Timedelta("30min"), periods=steps, freq="30min")
...
def run_diagnostics(s30: pd.Series, period: int = 48):
    """
    Quick sanity + backtest on the last 7 days.
    Uses shifts so prediction indexes align with the actuals.
    """
    if len(s30) < 8 * period:
        print("[diag] Not enough history for a 7-day backtest; skipping.")
        return

    # ---- 1) Missing-slot report (before interpolation) ----
    raw = pd.read_excel(
        EXCEL_PATH,
        sheet_name=SHEET_NAME,
        engine="openpyxl",
    ...
    grid = pd.date_range(raw.index.min().floor("30min"), raw.index.max().ceil("30min"), freq="30min")
    ...
    if len(missing):
        ...
        blocks = (miss.index.to_series().diff() != pd.Timedelta("30min")).cumsum()
        report = miss.groupby(blocks).agg(
            start=lambda s: s.index.min(), end=lambda s: s.index.max(), slots="count"
        )
        print("[diag] Largest missing blocks:\n", report.sort_values("slots", ascending=False).head(5))

    # ---- 2) 7-day backtest ----
    actual = s30.iloc[-7 * period:]  # last 7 days, strictly on the fixed 30-min grid

    # Seasonal-naive: yesterday's same slot -> shift by 'period'
    pred_sn = actual.shift(period)

    # K-day per-slot average (K=7): mean of the last 7 days same slot -> shifts 1..7*period
    shifts = [actual.shift(p * period) for p in range(1, 8)]
    pred_k7 = pd.concat(shifts, axis=1).mean(axis=1)

    # Metrics on aligned timestamps (drop the first 'period' where preds are NaN)
    def mape(y, yhat):
...
def main():
    ...
    s30 = load_and_regularise(EXCEL_PATH)
     ...
     # --- diagnostics (safe; handles indexing correctly) ---
    ...
    # Optional: save forecast + tail of history for inspection
